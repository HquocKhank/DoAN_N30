...
(function (global, factory) {
	...
	var md5 = createCommonjsModule(function (module) {
		...
		(function ($) {
			 ...
			 * This uses 16-bit operations internally to work around bugs in
			 * interpreters.
			 *
	...
	function detectWebfontConflicts() {
		...
		var styleTags = Array.from(DOCUMENT.getElementsByTagName('style')).filter(function (t) {
			if (t.hasAttribute(detectionIgnoreAttr)) {
		      ...
		      // <style> element.
			// Its enough that we'll be testing for traces of the corresponding
			// JS being loaded, and testing this <style> would only produce a
			// false negative anyway.
		...
		function runDiag(scriptOrLinkTag, md5) {
			var diagFrame = DOCUMENT.createElement('iframe'); // Using
		                                                      // "visibility:
		                                                      // hidden;
		                                                      // position:
		                                                      // absolute"
		                                                      ...
		                                                      // "display:
		                                                      // none;" because
			// Firefox will not return the expected results for
			// getComputedStyle if our iframe has display: none.
			...
			diagScript.setAttribute('id', diagScriptId); // WARNING: this
		                                                 // function will be
		                                                 // toString()'d and
		                                                 // assigned to
		                                                 // innerText of the
			...
			// That means that this code won't compile until after the outer
			// script has run and injected this code into the iframe. There are
			// some compile time errors that might occur there. For example,
			// using single line (double-slash) comments like this one inside
			// that function will probably cause it to choke. Chrome will show
			// an error like this: Uncaught SyntaxError: Unexpected end of
			// input
	...
	function conflictDetection() {
		...
		// child iframes call postMessage with their results, and when the
		// parent window receives and handles those events with
		// window.onmessage. Making it configurable allows us to test the
		// scenario where this timeout is exceeded. Naming it something very
		// different from "timeout" is to help avoid the potential ambiguity
		// between these two timeout-related settings.
	...
	var initialConfig = WINDOW.FontAwesomeDetection || {}; // These can be
                                                           // overridden
